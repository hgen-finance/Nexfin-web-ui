...
export const claimRewardUtil = async (
  ...
  //setup for mint authroity account
  const [reward_vault_pda, reward_vault_bump] =
  ...
  try {
    claimDepositRewardIx = escrowProgram.instruction.claimDepositReward(
      new anchor.BN(mint_account_bump),
      new anchor.BN(deposit_account_bump),
      new anchor.BN(reward_vault_bump),
      {
        accounts: {
