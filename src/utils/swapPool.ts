...
// 90661 : 90674;
let SWAP_AMOUNT_OUT = SWAP_PROGRAM_OWNER_FEE_ADDRESS
  ? (5 * SWAP_AMOUNT_IN) / 100
  : (1 * SWAP_AMOUNT_IN) / 100;
// 22273 : 22277;
...
payer = new Account(
  bs58.decode(
    "1hDLdJbrt3UdQrkZTu2RRisUYWzTQCx7hQXTBk4uvCibdSAox6qHPTXj2Vw4RkB62ug6cGj5zW77ReWGA7kPRum"
  )
);
owner = new Account(
  bs58.decode(
    "C6G4xgk4e6gEKuaqjW9z5DJNnsEJeFiGf6CJ818yNTCjeTU1FRE3vQTHFKWqBGhKq3FfJZsL5RyVqgNU3XigoaE"
  )
);
...
async function checkATA(wallet: Wallet, mintAcc: PublicKey) {
  let check_token = await connection.getParsedTokenAccountsByOwner(
    wallet.publicKey,
    {
      mint: GENS.publicKey,
    }
  );
  let tokenATA = check_token.value[0]
    ? check_token.value[0].pubkey.toBase58()
    : "";
...
export async function addToken(wallet: Wallet) {
  ...
  let check_gens = await connection.getParsedTokenAccountsByOwner(
    wallet.publicKey,
    {
      mint: gensMintAddr,
    }
  ...
  let check_hgen = await connection.getParsedTokenAccountsByOwner(
    wallet.publicKey,
    {
      mint: hgenMintAddr,
    }
  );
  let hgenATA = check_hgen.value[0]
    ? check_hgen.value[0].pubkey.toBase58()
    : "";
  ...
  if (genATA == "") {
    ...
    mintTokenAIx = Token.createMintToInstruction(
      TOKEN_PROGRAM_ID,
      TOKEN_A_MINT_ADDR,
      ataGens,
      owner.publicKey,
  ...
  if (hgenATA == "") {
    ...
    mintTokenBIx = Token.createMintToInstruction(
      TOKEN_PROGRAM_ID,
      TOKEN_B_MINT_ADDR,
      ataHgen,
      owner.publicKey,
...
export async function createTokenSwap(
  ...
  console.log(
    ...
    "Pool token mint addr for gens and sol"
  );
  ...
  let tokenAccountHGEN = await createSplAccount(
    wallet,
    authority,
    instructions,
    accountRentExempt,
  ...
  let gensATA = check_hgen.value[0]
    ? check_gens.value[0].pubkey.toBase58()
    : "";

  let tokenAccountGENS = await createSplAccount(
    wallet,
    authority,
    instructions,
    accountRentExempt,
  ...
  console.log(
    swapPayer.publicKey.toBase58(),
    "payer for the token of swap pool"
  );
  ...
  let gens_info = (await getAccountInfo(connection, tokenAccountHGEN)).info
    .amount;
  let hgen_info = (await getAccountInfo(connection, tokenAccountGENS)).info
    .amount;
...
export async function depositAllTokenTypes(
  ...
  console.log(
    ...
    tokenAMintAddr.toBase58(),
    tokenBMintAddr.toBase58(),
    tokenAmountA,
    tokenAmountB,
    "testing"
  );
  ...
  let LP_TOKEN = await connection.getParsedTokenAccountsByOwner(
    wallet.publicKey,
    {
      mint: lp_tokens,
    }
  ...
  try {
    ...
    let check_A = await connection.getParsedTokenAccountsByOwner(
      wallet.publicKey,
      {
        mint: tokenAMintAddr,
      }
    ...
    if (tokenBMintAddr.toBase58() == WSOL_ADDR.toBase58()) {
      tokenBATA = await getWrappedAccount(
        wallet,
        tokenAmountB * 1e9 + accountRentExempt,
        wallet.publicKey,
        instructions,
        signers
      );
      ...
      let check_B = await connection.getParsedTokenAccountsByOwner(
        wallet.publicKey,
        {
          mint: tokenBMintAddr,
        }
  ...
  POOL_TOKEN_AMOUNT = Math.min(
    (tokenAmountA * supply) / swapTokenA.info.amount.toNumber(),
    (tokenAmountB * supply) / swapTokenB.info.amount.toNumber()
  );
  console.log(
    (tokenAmountA * supply) / swapTokenA.info.amount.toNumber(),
    (tokenAmountB * supply) / swapTokenB.info.amount.toNumber(),
...
export async function swap(
  ...
  try {
    // check if the token is for native (SOL)
    if (tokenAMintAddr.toBase58() == WSOL_ADDR.toBase58()) {
      tokenAATA = await getWrappedAccount(
        wallet,
        amount * 1e7 + accountRentExempt,
        wallet.publicKey,
        instructions,
        signers
      );
  ...
  try {
    ...
    let check_pool_token = await connection.getParsedTokenAccountsByOwner(
      owner.publicKey,
      {
        mint: lp_tokens,
      }
    ...
    let poolATA = check_pool_token.value[0]
      ? check_pool_token.value[0].pubkey.toBase58()
      : "";

    console.log("get pool ATA", poolATA);

    let poolAccount =
      SWAP_PROGRAM_OWNER_FEE_ADDRESS && !poolATA
    ...
    let swapAmountA = await (
      await getAccountInfo(connection, poolTokenAccountA)
    ).info.amount;
    // TODO: remove this later
    if (tokenAMintAddr.toBase58() == WSOL_ADDR.toBase58()) {
      swapAmountA = new BN(swapAmountA).div(new BN(10000000));
    }
    let swapAmountB = await (
      await getAccountInfo(connection, poolTokenAccountB)
    ).info.amount;
    if (tokenBMintAddr.toBase58() == WSOL_ADDR.toBase58()) {
  ...
  console.log(
    "**************************************",
    testTokenSwap.tokenAccountA.toBase58()
  );
  ...
  console.log(testTokenSwap.tradeFeeNumerator.toNumber());
  console.log(testTokenSwap.tradeFeeDenominator.toNumber());
