...
import { TOKEN_GENS, TOKEN_HGEN, CLUSTER } from "@/utils/layout";
          ...
          await this.$axios
            ...
            .then(async ({ data }) => {
              if (data.model) {
                ...
                let result;
                try {
                  result = await program.account.deposit.fetch(
                ...
                commit(
                  "setRewardHgenAmount",
                  result.rewardGovernanceTokenAmount
                );
      ...
      if (value && Number(value.from) > 0) {
        if (!state.depositKey.deposit) {
          ...
          try {
            ...
            try {
              result = await program.account.deposit.fetch(
                new PublicKey(data.depositAccountPubkey)
              );
              console.log(result, "result");
            } catch (err) {
              console.error(err);
