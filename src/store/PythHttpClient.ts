...
export class PythHttpClient {
  ...
  public async getData(): Promise<PythHttpClientResult> {
      ...
      if (base) {
        switch (base.type) {
          ...
          case AccountType.Product:
            ...
            productAccountKeyToProduct.set(
              singleAccount.pubkey.toBase58(),
              productData.product
            );
    ...
    priceDataQueue.forEach((priceData) => {
      const product = productAccountKeyToProduct.get(
        priceData.productAccountKey.toBase58()
      );
