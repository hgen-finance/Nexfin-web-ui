...
import {
  ...
  TOKEN_ACC_SOL_HS,
  LP_TOKENS_GS,
  TOKEN_SWAP_HGEN_SOL_ACCOUNT,
  LP_TOKENS_HS,
  TOKEN_SWAP_GEN_SOL_ACCOUNT,
} from "@/utils/layout";
...
export const actions = actionTree(
  { state, getters, mutations },
  {
    useNativeAccount({ commit, state }, value) {
      ...
      const account = wrapNativeAccount(
    ...
    async createTokenSwapPool({ commit, dispatch, state }) {
      console.log("CreateTokenSwap (constant product)");
      let poolInfo: Pool = await createTokenSwap(
    ...
    async depositAllToken({ commit, state }, value) {
      ...
      try {
        await depositAllTokenTypes(
          ...
          value.tokenAacc,
          value.tokenBacc,
          value.tokenAMintAddr,
          value.tokenBMintAddr,
          value.from,
          value.to
        );
      } catch (err) {
    ...
    async withdrawToken({ commit, state }, value) {
      ...
      if (value.tokenType == "HS") {
        try {
          tokenSwapAccount = TOKEN_SWAP_HGEN_SOL_ACCOUNT;
          let LP_TOKEN = await this.$web3.getParsedTokenAccountsByOwner(
            this.$wallet.publicKey,
            {
              mint: LP_TOKENS_HS,
            }
          );
          let tokenATA = LP_TOKEN.value[0]
            ? LP_TOKEN.value[0].pubkey.toBase58()
            : "";
          console.log(tokenATA, "tokenATA");

          let LP_TOKEN_FEE = await this.$web3.getParsedTokenAccountsByOwner(
            new PublicKey("54sdQpgCMN1gQRG7xwTmCnq9vxdbPy8akfP1KrbeZ46t"),
            {
              mint: LP_TOKENS_HS,
            }
          );
          console.log(LP_TOKEN_FEE, "owner");
          let tokenATAFee = LP_TOKEN_FEE.value[0]
            ? LP_TOKEN_FEE.value[0].pubkey.toBase58()
            : "";
      ...
      if (value.tokenType == "GS") {
        ...
        try {
          ...
          swapTx = await swap(
            ...
            value.tokenAacc,
            value.tokenBacc,
            new PublicKey(value.tokenAMintAddr),
            new PublicKey(value.tokenBMintAddr),
            hostFeeAccount,
            value.from,
            value.slippagePrice
          );
    ...
    async getTokenAInfo({ state, commit }, value) {
      let tokenA = await this.$web3.getParsedTokenAccountsByOwner(
        POOL_AUTHORITY,
        { mint: TOKEN_A_MINT_ADDR }
      );
      let result: number =
        tokenA.value[0].account.data.parsed.info.tokenAmount.uiAmount;
      commit("setTokenAmountA", result);

      tokenA = await this.$web3.getParsedTokenAccountsByOwner(
        POOL_AUTHORITY_GS,
        { mint: TOKEN_A_MINT_ADDR }
      );
    ...
    async getTokenBInfo({ state, commit }, value) {
      let tokenB = await this.$web3.getParsedTokenAccountsByOwner(
        POOL_AUTHORITY,
        { mint: TOKEN_B_MINT_ADDR }
      );
      let result: number =
        tokenB.value[0].account.data.parsed.info.tokenAmount.uiAmount;
      commit("setTokenAmountB", result);

      tokenB = await this.$web3.getParsedTokenAccountsByOwner(
        POOL_AUTHORITY_GS,
        { mint: WSOL_ADDR }
      );
    ...
    async onTokenAChange({ dispatch }, value) {
      this.$web3.onAccountChange(TOKEN_ACC_A, () =>
        dispatch(
          "swapPool/getTokenAInfo",
          { authority: POOL_AUTHORITY, tokenAMintAddr: TOKEN_A_MINT_ADDR },
          { root: true }
        )
      );

      this.$web3.onAccountChange(TOKEN_ACC_GENS_GS, () =>
        dispatch(
          "swapPool/getTokenAInfo",
          { authority: POOL_AUTHORITY_GS, tokenAMintAddr: TOKEN_A_MINT_ADDR },
          { root: true }
        )
      );

      this.$web3.onAccountChange(TOKEN_ACC_HGEN_HS, () =>
        dispatch(
          "swapPool/getTokenAInfo",
          { authority: POOL_AUTHORITY_HS, tokenAMintAddr: TOKEN_B_MINT_ADDR },
          { root: true }
        )
      );
    },
    ...
    async onTokenBChange({ dispatch }, value) {
      this.$web3.onAccountChange(TOKEN_ACC_B, () =>
        dispatch(
          "swapPool/getTokenBInfo",
          { authority: POOL_AUTHORITY, tokenBMintAddr: TOKEN_A_MINT_ADDR },
          { root: true }
        )
      );
      this.$web3.onAccountChange(TOKEN_ACC_SOL_GS, () =>
        dispatch(
          "swapPool/getTokenBInfo",
          { authority: POOL_AUTHORITY_GS, tokenBMintAddr: WSOL_ADDR },
          { root: true }
        )
      );
      this.$web3.onAccountChange(TOKEN_ACC_SOL_HS, () =>
        dispatch(
          "swapPool/getTokenBInfo",
          { authority: POOL_AUTHORITY_HS, tokenBMintAddr: WSOL_ADDR },
          { root: true }
        )
      );
    },
