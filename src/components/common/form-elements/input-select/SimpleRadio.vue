<template>
  <div class="w-100 p-r">
    ...
    <div class="w-100 p-r radio-holder d-f" :class="{ 'fd-c': direction === 'column', 'fd-r': direction === 'row' }">
      <label v-for="(opt, index) in options" :key="opt" class="radio-container" :class="{'ml-2': index !== 0 && direction === 'row', 'mt-2': index !== 0 && direction === 'column'}">
        <span class="f-white-200 fw-400 fs-6">
          {{ opt }}
        </span>
        <input type="radio" :name="inputName" :value="opt" @change="$emit('input', opt)">
