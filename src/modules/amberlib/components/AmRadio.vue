  ...
  <div class="d-ib amRadio" :class="{disabled: modelData.disabled}">
    ...
    <div class="d-ib pt-3" :class="{'pt-3': modelData.label}">
      <div :class="{'fd-r': modelData.list, 'fd-c': !modelData.list}">
        <div class="fd-r ai-c amRadioContainer" v-for="(item, i) in modelData.items" :key="i" :class="{'ml-2': modelData.list && i > 0, 'mt-2': !modelData.list && i > 0}">
          <input type="radio" :value="item.value" class="w-100 h-100 p-a l-0 t-0 z-10" v-model="modelValue" :disabled="modelData.disabled" />
          <div class="w-fix-s-14 h-fix-s-14 br-6 brs-s fd-r ai-c jc-c amRadioBlock rad-50" :class="[getClassActive(item.value), getCheckedColor(item.value)]">
            <svg class="w-80 h-80" viewBox="0 0 120 120" version="1.1" xmlns="http://www.w3.org/2000/svg">
              <circle :class="[`svg-f-${modelData.colorBackgroundChecked}`]" cx="60" cy="60" r="50"/>
            </svg>
...
export default {
  ...
  methods: {
    ...
    getCheckedColor (value) {
      return value === this.modelValue ? `br-${this.modelData.colorBorderChecked} ${this.modelData.colorBackground}`: `br-${this.modelData.colorBorder} ${this.modelData.colorBackground}`
    }
  },
  watch: {
    modelValue (val) {
